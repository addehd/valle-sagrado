...
/**
 * One-time script to insert the "coaching" page into coach_pages table
 * Run with: node scripts/insert-coaching-page.mjs
 */

const coachingContent = `"OM MAN KAN UNDERVISA MED HUMOR
...
const payload = {
	...
	meta_description: 'Min coaching-filosofi kring träning, yoga, närvaro och näring.',
	is_published: true
};

console.log('Inserting coaching page...\n');
console.log('Title:', payload.title);
console.log('Slug:', payload.slug);
console.log('Content length:', payload.content.length, 'characters');
console.log('\n---\n');
console.log('To insert this page, you need to:');
console.log('1. Make sure you\'re logged in as an admin on your site');
console.log('2. Run this curl command:\n');
console.log(`curl -X POST http://localhost:5173/api/admin/coaching-pages \\
  -H "Content-Type: application/json" \\
  -H "Cookie: YOUR_SESSION_COOKIE" \\
  -d '${JSON.stringify(payload, null, 2)}'`);
console.log('\n---\n');
console.log('Or use the Supabase SQL editor to insert directly:');
console.log(`
INSERT INTO coach_pages (project_id, slug, title, content, meta_description, is_published)
SELECT id, 'coaching', 'Coaching', $$${coachingContent}$$, 'Min coaching-filosofi kring träning, yoga, närvaro och näring.', true
FROM projects WHERE name = 'YOUR_PROJECT_NAME' LIMIT 1;
`);
