...
export const load: PageServerLoad = async ({ locals: { supabase, user, safeGetSession }, params }) => {
	try {
		...
		if (!currentUser) {
			throw redirect(302, `/auth?redirect=/rikuy/${params.project}/admin/csv-upload`);
		}
		...
		console.error('CSV upload page error:', err);
		return {
			project: null,
...
export const actions: Actions = {
	upload: async ({ request, locals: { supabase, user, safeGetSession }, params }) => {
		try {
			// Get the current session and user
			const { session, user: sessionUser } = await safeGetSession();
			const currentUser = user || sessionUser;
			...
			const results = await parseCSVAndCreateProducts(csvText, project.id, currentUser.id, supabase);
