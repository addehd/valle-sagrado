...
<div class="container mx-auto px-4 py-8">
	...
	<p class="text-gray-600 dark:text-gray-400 mb-8">
		...
		{#if project}
			<span class="font-semibold text-blue-600 dark:text-blue-400">{project.name}</span>
		{:else}
			your project
		{/if}
	</p>
		...
		<div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mb-6">
			<div class="flex">
				...
				<div class="ml-3">
					...
					<p class="mt-1 text-sm text-yellow-700 dark:text-yellow-300">{(data as any).message}</p>
				</div>
			</div>
		</div>
	{/if}
